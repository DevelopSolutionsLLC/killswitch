...
- `check` stops the protected `SERVICE`, verifies it is down, waits up to `READINESS_TIMEOUT` seconds for OpenVPN, the tunnel interface, DNS, and the tunnel ping to be healthy before starting the protected `SERVICE` and entering the monitor loop. It pings `CHECK_HOST` through `NET_TUN`; on failure it stops the protected `SERVICE`, verifies the protected service is stopped, reapplies deny-by-default UFW policy, logs the failure, and intentionally forces a reboot.
